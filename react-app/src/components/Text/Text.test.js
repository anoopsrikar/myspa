...
describe("Text ->", () => {
    ...
    beforeEach(() => {
        sandbox.stub(ModelManager, 'getData')
            .withArgs({pagePath: CONTENT_PATH}).resolves({test: true});
    ...
    it("should render the text component with no parameter",() => {
        ...
        expect(rootNode.querySelector(RTE_EDIT_ELEMENT_DATA_ATTR_SELECTOR)).toBeNull();
    });
    ...
    it("should render the text as a rich text component with a given id",() => {
        ...
        expect(rootNode.querySelector(RTE_EDIT_ELEMENT_DATA_ATTR_SELECTOR)).not.toBeNull();
        expect(rootNode.querySelector('#' + extractModelId(CONTENT_PATH))).not.toBeNull();
    });
});
